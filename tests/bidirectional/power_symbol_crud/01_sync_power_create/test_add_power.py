#!/usr/bin/env python3
"""
Test 34: Add Power Symbol - Basic Generation Test

Validates that adding a power symbol (power net)
...
def test_34_add_power(request):
    """Test adding power symbol - basic generation."""
    ...
    try:
        ...
        # Verify power symbols exist (VCC, GND)
        power_components = [c for c in root_sch.components if c.reference.startswith("#PWR")]
        assert len(power_components) >= 2, "Expected at least 2 power symbols"

        print("\n✅ Test 34 PASSED: Power symbol test circuit generated successfully")
        print(f"   - Root sheet: 3 components (R1, R2, C1) ✓")
        print(f"   - Power symbols: {len(power_components)} (VCC, GND) ✓")
        print(f"\n💡 Note: Full test includes adding +3V3 power symbol and verifying positions")
