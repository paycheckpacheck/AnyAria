#!/usr/bin/env python3
"""
Test 36: Rename Power Net - Basic Generation Test

Validates that renaming a power net (e.g., VCC to VBAT)
...
def test_36_rename_power(request):
    """Test renaming power net - basic generation."""
    ...
    try:
        ...
        assert len(power_components) >= 2, "Expected at least 2 power symbols (VCC, GND)"

        print("\n✅ Test 36 PASSED: Power rename test circuit generated successfully")
        print(f"   - Root sheet: 3 components (R1, R2, C1) ✓")
        print(f"   - Power symbols: {len(power_components)} (VCC, GND) ✓")
        print(f"\n💡 Note: Full test includes renaming VCC to VBAT and verifying positions")
