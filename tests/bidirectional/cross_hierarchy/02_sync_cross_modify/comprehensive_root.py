#!/usr/bin/env python3
"""
Comprehensive root circuit for Test 39 - Modify cross-sheet connection.

Hierarchy:
- Root sheet with R1, R2
- Amplifier subcircuit with R3, R4
- Power supply subcircuit with R5, C2
- Will change power_supply connection from sig_out to sig_in

Test operation: Change power_supply control_sig from sig_out to sig_in
Expected: Cross-sheet connection modified, all positions preserved
"""
...
def comprehensive_root():
    ...
    power_supply(control_sig=sig_out, vcc=vcc, gnd=gnd)  # Will change to sig_in
...
if __name__ == "__main__":
    ...
    print("  - Cross-sheet connection: sig_out → power_supply")
    print("\n💡 Change power_supply(control_sig=sig_out) to sig_in to test")
