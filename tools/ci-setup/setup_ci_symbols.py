...
def download_file(url, output_path):
    ...
    try:
        ...
        req = urllib.request.Request(url, headers={
            'User-Agent': 'circuit-synth CI setup'
        })
        ...
        print(f"✅ Successfully downloaded {output_path.name} ({output_path.stat().st_size:,} bytes)")
        return True
...
def main():
    ...
    ci_detected = any(env in os.environ for env in ['CI', 'GITHUB_ACTIONS', 'GITLAB_CI', 'TRAVIS', 'CIRCLECI'])
    if ci_detected:
